...
describe Users::SessionsController do
  ...
  describe "Devise lock" do
    context "when devise sign in maximum_attempts reached", :consul do
  ...
  describe "access logs" do
    context "when feature is enabled", :consul do
      ...
      it "when a sign in process fails it calls the authentication logger", :consul do
      ...
      it "when maximum attempts is reached it tracks the user account lock", :consul do
        allow(User).to receive(:maximum_attempts).and_return(1)
    ...
    context "when feature is disabled" do
      ...
      it "when a sign in process succeeds it does not call the authentication logger", :consul do
        expect(AuthenticationLogger).not_to receive(:log)

        post :create, params: { user: { login: "citizen@consul.org", password: "12345678" }}
      end

      it "when a sign in process fails it does not call the authentication logger", :consul do
        expect(AuthenticationLogger).not_to receive(:log)
