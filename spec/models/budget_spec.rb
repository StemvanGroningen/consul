...
describe Budget do
  ...
  describe "info_section_link_url" do
    it "is not required if info_section_link_text is not provided" do
      valid_budget = build(:budget, info_section_link_text: nil)

      expect(valid_budget).to be_valid
    end

    it "is required if info_section_link_text is provided" do
      invalid_budget = build(:budget, info_section_link_text: "link text")

      expect(invalid_budget).not_to be_valid
      expect(invalid_budget.errors.count).to be 1
      expect(invalid_budget.errors[:info_section_link_url].count).to be 1
      expect(invalid_budget.errors[:info_section_link_url].first).to eq "can't be blank"
    end

    it "is valid if info_section_link_text and info_section_link_url are both provided" do
      valid_budget = build(:budget, info_section_link_text: "Text link",
                                    info_section_link_url: "https://info.consulproject.org")
